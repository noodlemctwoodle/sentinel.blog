...
# The Preview API Version is needed to include the MITRE ATT&CK "Sub techniques" in the rule templates
...
# Helper function to update Analytic Rule
...
# Helper function to update Metadata for Analytic rule
...
# Helper function to convert entity mappings to a hash table for easier comparison
function ConvertToHashTable($mappings) {
    $hashTable = @{}
    foreach ($mapping in $mappings) {
        # Validate that entityType exists and is not null
        if ($null -ne $mapping.entityType) {
            $fieldMappings = @()

            # Ensure fieldMappings is always treated as an array
            if ($mapping.fieldMappings -is [Array]) {
            ...
            $hashTable[$mapping.entityType] = $fieldMappings
        }
        else {
            Write-Warning "Skipping a mapping with null entityType."
        }
    }
    return $hashTable
}
...
foreach ($contentTemplate in $matchedTemplates) { 
    ...
    if ($templateVersion.version -notin $ruleTemplateVersion.templateVersion) {
        Write-Output "Update available - a new version of the Analytic rule [$($ruleResponse.properties.displayName)] is available..."
        Write-Output "Updating the analytic rule..."        
        ...
        # Load the existing (pre-updated) Analytic rule entity mappings
        $preUpdatedRuleEntityMappings = ($ruleSkipTuned | Where-Object alertRuleTemplateName -eq $ruleContentId | Select-Object entityMappings).entityMappings   

        # Load the existing (pre-updated) Analytic rule custom details
        $preUpdatedRuleCustomDetails = ($ruleSkipTuned | Where-Object alertRuleTemplateName -eq $ruleContentId | Select-Object customDetails).customDetails    
        
        # Load the latest Analytic rule entity mappings
        $ruleEntityMappings = ($ruleResponse.properties.mainTemplate.resources.properties | Select-Object entityMappings).entityMappings    
        ...
        # Retain entities from the current mappings that are not in the new mappings
        foreach ($entityType in $currentMappings.Keys) {
          if (-not $newMappings.ContainsKey($entityType)) {
             $finalMappings += [PSCustomObject]@{
                entityType    = $entityType
                fieldMappings = $currentMappings[$entityType]
             }
         }
     }

        # Retain custom details from the pre-updated rule
        $finalCustomDetails = $preUpdatedRuleCustomDetails

        # Output results
        Write-Output "`nFinal Entity Mappings:"
        $finalMappings | ForEach-Object {
          Write-Output "EntityType: $($_.entityType)"
           $_.fieldMappings | ForEach-Object {
            Write-Output "  Identifier: $($_.identifier), ColumnName: $($_.columnName)"
          }
      }

        Write-Output "`nFinal Custom Details:"
        $finalCustomDetails.PSObject.Properties | ForEach-Object {
        ...
        if ($null -ne $finalCustomDetails) {
            $rule.properties | Add-Member -NotePropertyName customDetails -NotePropertyValue $finalCustomDetails
        }    
        # Remove the existing entityMappings property if it exists
        if ($rule.properties.PSObject.Properties.Match('entityMappings')) {
            $rule.properties.PSObject.Properties.Remove('entityMappings')
            Write-Output "Removed existing 'entityMappings'..."
        }
        # Add the updated finalMappings as the new entityMappings
        $rule.properties | Add-Member -MemberType NoteProperty -Name 'entityMappings' -Value $finalMappings
        # Enable the rule
        $rule.properties.enabled = $true
        # Convert the new rule to JSON
